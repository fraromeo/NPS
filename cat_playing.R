...
clusters <- rect.hclust(dendo, k=10)
cluster <- cutree(dendo, k=10)

# try to merge features
first <- games[2,]
cluster.like <- numeric(length(unique(cluster)))
#names(cluster.like) <- names(cluster)

for (cl.ind in 1:length(cluster.like)){
  cl.names = names(cluster[cluster == cl.ind])
